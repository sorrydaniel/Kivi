...
def series():
    edasi3 = ttk.Button(raam, text="OTSI", style='my.TButton', command=vastus)
...
def vastus():
    ...
    aken.geometry("1700x720")
    tahvel2 = Canvas(aken, width=1700, height=720, background="black")
    tahvel2.grid()
    
    logo2 = PhotoImage(file="flixnet.png")
    img = tahvel2.create_image(400, 10, image=logo2, anchor = NW)
    
    s = ttk.Style()
    s.configure('my.TButton', font=('Roboto', 12, "bold"))
    
    tagasi = ttk.Button(aken, text="To Main Page", style='my.TButton', command=restart_program)
    tagasi.place(x=120, y=680, width=120, height=40)
    
    exitButton2 = ttk.Button(aken, text="QUIT", style='my.TButton', command=clickExitButton2)
    exitButton2.place(x=0, y=680, width=120, height=40)
